...
const copy = {
  en: {
    ...
    // Auth
    goodToSeeYou: "Good to see you.", loginSub: "Log in to continue where your kitchen left off.", emailAddress: "Email address", password: "Password", enterDashboard: "Enter dashboard", signingIn: "Signing in…", newHere: "New here?", welcomeBack: "WELCOME BACK", otpKicker: "VERIFY IT'S YOU", checkYourEmail: "Check your email.", otpSentTo: "We sent a 6-digit code to", otpCodeLabel: "Verification code", verifying: "Verifying…", verifyAndContinue: "Verify & continue", useDifferentEmail: "Use a different email", otpSentToast: "Verification code sent to your email",
    ...
    billingEyebrow: "RESTAURANT / ACCOUNT", billingTitle: "Your plan", billingSub: "A simple plan for a more thoughtful kitchen.", currentStatus: "CURRENT STATUS", statusActive: "Active", statusPending: "Pending verification", statusInactive: "Inactive", planPrefix: "Plan:", noPlanYet: "No plan selected yet", choosePrefix: "Choose", selected: "Selected", payByQr: "PAY BY QR", scanToPay: "Scan to pay, then confirm below", upiIdLabel: "UPI ID:", paymentRefLabel: "Payment reference / UTR number", paymentRefPh: "e.g. UPI txn ID", subscriptionOptionalNote: "Subscribing is entirely optional — Basic and Pro get the same dashboard as the free plan today. It just helps support Fedd.", ivePaidButton: "I've paid", paidDialogKicker: "CONFIRM YOUR PAYMENT", paidDialogTitle: "Have you paid?", paidDialogSub: "Add your reference note and a screenshot of the payment so an admin can verify it.", paymentProofLabel: "Screenshot of payment", confirmPaymentButton: "Yes, I've paid", paymentSubmittedToast: "Payment submitted — an admin will verify it shortly", subscriptionPromoKicker: "OPTIONAL", subscriptionPromoTitle: "Get a subscription", subscriptionPromoText: "Support Fedd with a Basic or Pro plan — your dashboard stays exactly the same either way.", getSubscriptionButton: "Get a subscription",
    // NGO
  ...
  hi: {
    ...
    goodToSeeYou: "आपको देखकर अच्छा लगा।", loginSub: "अपनी रसोई जहाँ छोड़ी थी वहीं से जारी रखने के लिए लॉग इन करें।", emailAddress: "ईमेल पता", password: "पासवर्ड", enterDashboard: "डैशबोर्ड खोलें", signingIn: "लॉग इन हो रहा है…", newHere: "नए हैं?", welcomeBack: "वापसी पर स्वागत है", otpKicker: "पुष्टि करें कि यह आप हैं", checkYourEmail: "अपना ईमेल जांचें।", otpSentTo: "हमने 6 अंकों का कोड भेजा है", otpCodeLabel: "सत्यापन कोड", verifying: "सत्यापित हो रहा है…", verifyAndContinue: "सत्यापित करें और जारी रखें", useDifferentEmail: "अलग ईमेल का उपयोग करें", otpSentToast: "सत्यापन कोड आपके ईमेल पर भेजा गया",
    ...
    billingEyebrow: "रेस्तरां / खाता", billingTitle: "आपकी योजना", billingSub: "अधिक विचारशील रसोई के लिए एक सरल योजना।", currentStatus: "वर्तमान स्थिति", statusActive: "सक्रिय", statusPending: "सत्यापन लंबित", statusInactive: "निष्क्रिय", planPrefix: "योजना:", noPlanYet: "अभी तक कोई योजना नहीं चुनी गई", choosePrefix: "चुनें", selected: "चयनित", payByQr: "QR से भुगतान करें", scanToPay: "भुगतान के लिए स्कैन करें, फिर नीचे पुष्टि करें", upiIdLabel: "UPI आईडी:", paymentRefLabel: "भुगतान संदर्भ / UTR नंबर", paymentRefPh: "उदा. UPI लेनदेन आईडी", subscriptionOptionalNote: "सदस्यता लेना पूरी तरह वैकल्पिक है — बेसिक और प्रो को आज फ्री प्लान जैसा ही डैशबोर्ड मिलता है। इससे बस Fedd को सहयोग मिलता है।", ivePaidButton: "मैंने भुगतान कर दिया", paidDialogKicker: "अपने भुगतान की पुष्टि करें", paidDialogTitle: "क्या आपने भुगतान कर दिया?", paidDialogSub: "अपना संदर्भ नोट और भुगतान का स्क्रीनशॉट जोड़ें ताकि एक एडमिन इसे सत्यापित कर सके।", paymentProofLabel: "भुगतान का स्क्रीनशॉट", confirmPaymentButton: "हाँ, मैंने भुगतान कर दिया", paymentSubmittedToast: "भुगतान सबमिट हो गया — एडमिन जल्द ही सत्यापित करेंगे", subscriptionPromoKicker: "वैकल्पिक", subscriptionPromoTitle: "सदस्यता लें", subscriptionPromoText: "बेसिक या प्रो योजना के साथ Fedd को सहयोग दें — आपका डैशबोर्ड बिल्कुल वैसा ही रहेगा।", getSubscriptionButton: "सदस्यता लें",
...
function App() {
  ...
  // Login is two steps now: a password check that triggers an emailed OTP code (requestLoginOtp),
  // then the code itself exchanges for a real session (completeLogin).
  const requestLoginOtp = async (email, password) => {
    await api.auth.login(email, password);
  };

  const completeLogin = async (email, token) => {
    const result = await api.auth.verifyOtp(email, token);
  ...
  const handleSignup = async (r, payload) => {
    ...
    await requestLoginOtp(payload.email, payload.password);
    setLoginRole(r);
  ...
  if (role === "landing") return <Landing showLogin={(r) => { setLoginRole(typeof r === "string" ? r : "restaurant"); setPendingOtpEmail(null); setShowLogin(true); }} hideLogin={() => { setShowLogin(false); setPendingOtpEmail(null); }} showSignup={(r) => { setShowLogin(false); setSignupRole(r); }} hideSignup={() => setSignupRole(null)} onSignup={handleSignup} onRequestOtp={requestLoginOtp} onCompleteLogin={completeLogin} signup={signupRole} dark={dark} setDark={setDark} lang={lang} setLang={setLang} login={showLogin} loginRole={loginRole} pendingOtpEmail={pendingOtpEmail} showAboutPage={() => setShowAbout(true)} showTermsPage={() => setShowTerms(true)} />;
...
function LoginModal({ t, onRequestOtp, onCompleteLogin, close, showSignup, role, initialEmail }) {
  ...
  const [step, setStep] = useState(initialEmail ? "otp" : "password");
  const [email, setEmail] = useState(initialEmail || "");
  const [password, setPassword] = useState("");
  const [otp, setOtp] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const submitPassword = async () => { if (!email || !password) return toast.error("Enter your email and password"); setSubmitting(true); try { await onRequestOtp(email, password); setStep("otp"); toast.success(t.otpSentToast); } catch (err) { toast.error(err.message || "Login failed"); } finally { setSubmitting(false); } };
  const submitOtp = async () => { if (!otp) return toast.error("Enter the code sent to your email"); setSubmitting(true); try { await onCompleteLogin(email, otp); } catch (err) { toast.error(err.message || "Invalid code"); } finally { setSubmitting(false); } };
  return <div className="modal-backdrop" onClick={(e) => { if (e.target === e.currentTarget) close(); }}><div className="login-modal"><button data-testid="close-login-button" className="modal-close" aria-label="Close login" onClick={close}><X size={18} /></button>{step === "password" ? <><div className="section-kicker">{t.welcomeBack} · {roleLabel}</div><h2>{t.goodToSeeYou}</h2><p>{t.loginSub}</p><label>{t.emailAddress}<input data-testid="login-email-input" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={emailPh} /></label><label>{t.password}<input data-testid="login-password-input" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" /></label><button data-testid="login-submit-button" className="primary-btn full" disabled={submitting} onClick={submitPassword}>{submitting ? t.signingIn : t.enterDashboard} <ArrowRight size={16} /></button>{!isAdmin && <div className="signup-line"><span>{t.newHere}</span><button data-testid="login-signup-link" onClick={() => showSignup && showSignup(role)}>{t.signUp}</button></div>}</> : <><div className="section-kicker">{t.otpKicker}</div><h2>{t.checkYourEmail}</h2><p>{t.otpSentTo} <strong>{email}</strong></p><label>{t.otpCodeLabel}<input data-testid="login-otp-input" value={otp} onChange={(e) => setOtp(e.target.value)} placeholder="123456" inputMode="numeric" /></label><button data-testid="login-otp-submit-button" className="primary-btn full" disabled={submitting} onClick={submitOtp}>{submitting ? t.verifying : t.verifyAndContinue} <ArrowRight size={16} /></button><button data-testid="login-otp-back-button" className="text-button full" style={{ marginTop: 14, justifyContent: "center" }} onClick={() => { setStep("password"); setOtp(""); }}>{t.useDifferentEmail}</button></>}</div></div>;
}
...
function BillingPage({ t, onShowTerms, setPage }) {
  ...
  const submit = async (e) => {
    ...
    if (!proofFile) return toast.error("Attach a screenshot of your payment");
    setSubmitting(true);
    try {
      const updated = await api.billing.submit(selectedPlan, reference);
      setSubscription(updated);
      await api.billing.uploadProof(proofFile);
  ...
  return <>
    ...
    <p className="billing-optional-note">{t.subscriptionOptionalNote}</p>
    ...
    <div className="tool-card">
      ...
      <button data-testid="ive-paid-button" className="primary-btn" onClick={() => setShowPaidDialog(true)}>{t.ivePaidButton}</button>
    </div>
    {showPaidDialog && <div className="modal-backdrop" onClick={(e) => { if (e.target === e.currentTarget) setShowPaidDialog(false); }}>
      <div className="login-modal paid-dialog">
        <button data-testid="close-paid-dialog-button" className="modal-close" aria-label="Close" onClick={() => setShowPaidDialog(false)}><X size={18} /></button>
        <div className="section-kicker">{t.paidDialogKicker}</div>
        <h2>{t.paidDialogTitle}</h2>
        <p>{t.paidDialogSub}</p>
        <form onSubmit={submit}>
          ...
          <label>{t.paymentProofLabel}<input data-testid="payment-proof-input" type="file" accept="image/*" onChange={(e) => setProofFile(e.target.files[0] || null)} />{proofFile && <small>{t.attachedPrefix} {proofFile.name}</small>}</label>
          <button data-testid="submit-payment-button" className="primary-btn full" type="submit" disabled={submitting}>{submitting ? t.submitting : t.confirmPaymentButton}</button>
        </form>
      </div>
    </div>}
  </>;
...
function AdminPage({ t, page }) {
  ...
  const loadRestaurants = () => api.admin.restaurants().then(setRestaurants).catch((err) => toast.error(err.message));

  useEffect(() => {
    setLoading(true);
    Promise.all([loadQueue(), loadSubscribers(), loadRescues(), loadPayments(), loadRestaurants()]).finally(() => setLoading(false));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const signupsByDay = useMemo(() => {
    ...
    return Object.entries(counts).sort(([a], [b]) => a.localeCompare(b)).map(([day, count]) => ({ day: day.slice(5), count }));
  }, [restaurants]);
