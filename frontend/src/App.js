...
function RestaurantTools({ t, logs }) { const [outlet, setOutlet] = useState("Andheri West"); const exportReport = () => { const csv = "Item,Category,Quantity kg,Cost impact\n" + logs.map((x) => `${x.item},${x.category},${x.qty},${x.cost}`).join("\n"); const url = URL.createObjectURL(new Blob([csv], { type: "text/csv" })); const a = document.createElement("a"); a.href = url; a.download = "wastelytics-monthly-report.csv"; a.click(); URL.revokeObjectURL(url); toast.success("Monthly report exported"); }; return <div className="tools-grid"><div className="tool-card"><div className="section-kicker">{t.multiOutlet.toUpperCase()}</div><h3>{t.compareKitchens}</h3><p>{t.switchContext}</p><select data-testid="outlet-selector" value={outlet} onChange={(e) => { setOutlet(e.target.value); toast.success(`${e.target.value} selected`); }}><option>Andheri West</option><option>Bandra East</option><option>Powai</option></select><span className="tool-note">{t.currentView} {outlet}</span></div><div className="tool-card"><div className="section-kicker">{t.benchmark.toUpperCase()}</div><h3>{t.belowPeers}</h3><p>{t.benchmarkText}</p><button data-testid="benchmark-detail-button" className="link-btn" onClick={() => toast.info("Benchmark compares anonymized category averages")}>{t.benchmarkDetails} <ArrowRight size={14} /></button></div><div className="tool-card"><div className="section-kicker">{t.reporting.toUpperCase()}</div><h3>{t.shareProgress}</h3><p>{t.exportSummary}</p><button data-testid="export-report-button" className="secondary-btn" onClick={exportReport}>{t.exportCsv} <ArrowRight size={14} /></button></div><div className="tool-card referral-card"><div className="section-kicker">{t.referSave.toUpperCase()}</div><h3>{t.giveMonthGetMonth}</h3><p>{t.referText}</p><button data-testid="referral-button" className="secondary-btn" onClick={() => { toast.success("Referral link ready to share"); }}>{t.copyReferral} <ArrowRight size={14} /></button></div></div>; }
function InventoryWatch({ t }) { const [items, setItems] = useState([{ name: "Paneer", current: 4, min: 8, unit: "kg" }, { name: "Basmati rice", current: 18, min: 12, unit: "kg" }, { name: "Garlic naan", current: 9, min: 15, unit: "packs" }]); const [ordered, setOrdered] = useState({}); const updateMin = (name, value) => setItems(items.map((x) => x.name === name ? { ...x, min: Number(value) } : x)); const reorder = (item) => { setOrdered({ ...ordered, [item.name]: true }); toast.success(`${item.name} added to reorder list`); }; return <div className="inventory-panel data-panel"><div className="panel-head"><div><h3>{t.inventoryTitle}</h3><span>{t.inventorySub}</span></div><span className="expiry-count"><Bell size={14} /> {items.filter((x) => x.current < x.min && !ordered[x.name]).length} {t.invAlerts}</span></div><div className="inventory-table"><div className="inventory-head"><span>{t.invItem}</span><span>{t.invCurrent}</span><span>{t.invMinimum}</span><span>{t.invAction}</span></div>{items.map((item) => { const low = item.current < item.min && !ordered[item.name]; return <div className={`inventory-row ${low ? "low" : ""}`} key={item.name}><b>{item.name}</b><span>{item.current} {item.unit}</span><label><input data-testid={`minimum-threshold-${item.name.toLowerCase().replaceAll(" ", "-")}`} type="number" min="0" value={item.min} onChange={(e) => updateMin(item.name, e.target.value)} /> {item.unit}</label>{ordered[item.name] ? <span className="reorder-done"><Check size={14} /> {t.invRequested}</span> : low ? <button data-testid={`reorder-${item.name.toLowerCase().replaceAll(" ", "-")}-button`} className="reorder-btn" onClick={() => reorder(item)}><Bell size={14} /> {t.invReorder} {item.name}</button> : <span className="stock-ok"><Check size={14} /> {t.invStockHealthy}</span>}</div>; })}</div><div className="inventory-tip"><Sparkles size={15} /> {t.invTip}</div></div>; }
function ExpiryWatch({ t, logs }) { const watch = [{ item: "Paneer tikka", status: "Expires today", qty: "6 kg", tone: "urgent" }, { item: "Garlic naan", status: "Expired yesterday", qty: "4 kg", tone: "expired" }, { item: "Dal makhani", status: "Use within 2 days", qty: "8 kg", tone: "soon" }]; return <div className="expiry-panel data-panel"><div className="panel-head"><div><h3>{t.expiryTitle}</h3><span>{t.expirySub}</span></div><span className="expiry-count"><Clock3 size={14} /> 2 {t.expiryNeedAction}</span></div><div className="expiry-grid">{watch.map((x) => <div className="expiry-item" key={x.item}><div className="expiry-icon"><Clock3 size={16} /></div><div><b>{x.item}</b><small>{x.qty} · {x.status}</small></div><button data-testid={`expiry-action-${x.item.toLowerCase().replaceAll(" ", "-")}`} className={`expiry-action ${x.tone}`} onClick={() => toast.success(x.tone === "expired" ? `${x.item} marked for waste review` : `${x.item} added to today’s action list`)}>{x.tone === "expired" ? t.expiryLogWaste : t.expiryPlanUse}</button></div>)}</div><div className="expiry-tip"><Sparkles size={15} /> {t.expiryTip}</div></div>; }
function WastePlaybook({ t, logs = [] }) { const counts = logs.reduce((acc, x) => ({ ...acc, [x.item]: (acc[x.item] || 0) + Number(x.qty) }), {}); const topItem = Object.entries(counts).sort((a, b) => b[1] - a[1])[0] || ["Paneer tikka", 18]; const reduction = Math.max(10, Math.min(25, Math.round(topItem[1] / 2))); return <div className="playbook-panel"><div className="playbook-copy"><div className="section-kicker">{t.playbookKicker}</div><h3>{t.playbookReduceBy} {topItem[0]} {t.playbookPrepBy}{reduction}%.</h3><p>{t.playbookHistoryPrefix} {topItem[1]} {t.playbookHistorySuffix} {topItem[0]} {t.playbookSurplusSuffix}</p><button data-testid="playbook-create-button" className="secondary-btn" onClick={() => toast.success(`Prep checklist created for ${topItem[0]}`)}>{t.playbookCreateChecklist} <ArrowRight size={15} /></button></div><div className="playbook-steps"><div><span>1</span><b>{t.playbookStep1}</b><small>−{reduction}% {topItem[0]}</small></div><div><span>2</span><b>{t.playbookStep2}</b><small>{t.playbookStep2Sub}</small></div><div><span>3</span><b>{t.playbookStep3}</b><small>{t.playbookStep3Sub}</small></div></div></div>; }
...
const copy = {
  en: {
    overview: "Overview", log: "Log surplus", market: "Marketplace", billing: "Billing", sub: "Here’s what your kitchen is telling you this week.", wasted: "Total food wasted", impact: "Cost impact estimate", top: "Top wasted item", trend: "Avg per entry", updated: "Live · updated 2 min ago", insights: "Actionable insights", week: "This week", month: "This month", recent: "Recent logs", viewAll: "View all", add: "Log food waste", recommendations: "Recommendations", marketTitle: "Surplus marketplace", claim: "Claim food", claims: "My claims", login: "Restaurant login", liveListings: "Live listings", mapDistance: "Map & distance", verificationStatus: "Verification status", filterAll: "All", filterHigh: "High priority", filterVeg: "Vegetarian", filterCold: "Refrigerated", rescueOpps: "Rescue opportunities", inventoryTitle: "Inventory reorder watch", inventorySub: "Set minimums before a stock gap affects service", expiryTitle: "Expiry watch", expirySub: "Protect margin before food goes out of date", benchmark: "Benchmark", reporting: "Reporting", referSave: "Refer & save", multiOutlet: "Multi-outlet view", exitDemo: "Exit demo", currentPlan: "Current plan",
    // Landing
    navHow: "How it works", navPricing: "Pricing", navAbout: "About", ngoSignupBtn: "NGO sign up", heroEyebrow: "Data-led kitchen intelligence", heroTitle1: "Turn food waste", heroTitleEm: "into savings.", heroSub: "Wastelytics gives restaurants the visibility to understand waste, act on patterns, and keep more money on every plate.", startTrial: "Start free trial", joinNgo: "Join as NGO", heroProof: "Built for kitchens that care about every gram", ticketLabel: "WASTELYTICS / WEEKLY PULSE", ticketLive: "LIVE", ticketSavings: "EST. SAVINGS THIS MONTH", ticketChangeText: "18.4% less waste", ticketTrendLabel: "WASTE TREND · 8 WEEKS", loopKicker: "THE LOOP", loopTitle1: "Less waste is a", loopTitleSpan: "measurable advantage.",
    ...
    factsItems: [["1/3", "of food produced globally is lost or wasted.", "UNEP Food Waste Index · sourced"], ["₹25k Cr", "estimated annual food-waste cost across Indian restaurants.", "Industry estimate · clearly labelled"], ["18.4%", "less waste for the sample kitchen after using weekly insights.", "Wastelytics demo estimate"]],
    aboutKicker: "ABOUT WASTELYTICS", aboutTitle: "About", contentComingSoon: "Content coming soon.", aboutBackHome: "Back to home", founderTitle: "Founder, Wastelytics", founderTagline: "Building a data-driven path from food waste to food rescue.", aboutMissionLine: "Less waste. More reach. One dashboard at a time.", aboutCta: "See how it works", bySubscribingText: "By subscribing, you agree to our", trustKicker: "YOUR DATA, YOUR DECISIONS", trustLine: "Simple tools for more thoughtful kitchens.", termsLink: "Terms & conditions", termsKicker: "WASTELYTICS · TERMS", close: "Close", footerTagline: "Waste less. Know more.", restaurantSignup: "Restaurant sign up", ngoLogin: "NGO login", adminSignup: "Admin sign up", adminLogin: "Admin login",
    // Portal-wide
    ...
    signUp: "SIGN UP", iAmA: "I am a...", roleRestaurant: "Restaurant", roleNgo: "NGO", roleAdmin: "Admin", createAccount: "Create your account", restaurantNamePh: "e.g. Green Tiffin Co.", ngoNamePh: "e.g. Robin Hood Army", adminNamePh: "e.g. Priya Rao", restaurantNameLabel: "Restaurant name", ngoNameLabel: "Organisation name", adminNameLabel: "Full name", city: "City", contactPerson: "Contact person", fullName: "Full name", adminInviteCode: "Admin invite code", locationLink: "Location link (Google Maps)", locationHint: "Helps NGOs plan the shortest pickup route.", ngoNote: "Applications are reviewed by the Wastelytics team. Verified NGOs typically hear back within 48 hours.", adminNote: "Admin access requires a valid invite code from the Wastelytics team.", submitting: "Submitting…", submitVerification: "Submit for verification", createAdminAccess: "Create admin access", ngoDesc: "Verified NGOs can claim surplus food from restaurants in your city.", adminDesc: "Admin access is limited to Wastelytics team members with a valid invite code.", restaurantDesc: "Sign up in under a minute — no card needed for the free trial.", ngoHeadline: "Rescue surplus, feed more.", adminHeadline: "Command the platform.", restaurantHeadline: "Turn waste into savings.",
    // LogPage
  ...
  hi: {
    ...
    navHow: "यह कैसे काम करता है", navPricing: "मूल्य निर्धारण", navAbout: "हमारे बारे में", ngoSignupBtn: "NGO साइन अप", heroEyebrow: "डेटा-आधारित रसोई इंटेलिजेंस", heroTitle1: "भोजन की बर्बादी को", heroTitleEm: "बचत में बदलें।", heroSub: "Wastelytics रेस्तरां को बर्बादी समझने, पैटर्न पर कार्रवाई करने और हर थाली से अधिक पैसा बचाने की स्पष्टता देता है।", startTrial: "मुफ़्त ट्रायल शुरू करें", joinNgo: "NGO के रूप में जुड़ें", heroProof: "हर ग्राम की परवाह करने वाली रसोई के लिए बनाया गया", ticketLabel: "WASTELYTICS / साप्ताहिक पल्स", ticketLive: "लाइव", ticketSavings: "इस महीने अनुमानित बचत", ticketChangeText: "18.4% कम बर्बादी", ticketTrendLabel: "बर्बादी रुझान · 8 सप्ताह", loopKicker: "प्रक्रिया", loopTitle1: "कम बर्बादी एक", loopTitleSpan: "मापने योग्य लाभ है।",
    ...
    factsItems: [["1/3", "विश्व स्तर पर उत्पादित भोजन का हिस्सा बर्बाद हो जाता है।", "UNEP फूड वेस्ट इंडेक्स · स्रोत सहित"], ["₹25k करोड़", "भारतीय रेस्तरां में अनुमानित वार्षिक खाद्य-अपशिष्ट लागत।", "उद्योग अनुमान · स्पष्ट रूप से लेबल किया गया"], ["18.4%", "साप्ताहिक जानकारी का उपयोग करने के बाद नमूना रसोई के लिए कम बर्बादी।", "Wastelytics डेमो अनुमान"]],
    aboutKicker: "WASTELYTICS के बारे में", aboutTitle: "हमारे बारे में", contentComingSoon: "सामग्री जल्द आ रही है।", aboutBackHome: "होम पर वापस जाएं", founderTitle: "संस्थापक, Wastelytics", founderTagline: "भोजन की बर्बादी से भोजन बचाव तक का एक डेटा-आधारित रास्ता बनाना।", aboutMissionLine: "कम बर्बादी। अधिक पहुंच। एक समय में एक डैशबोर्ड।", aboutCta: "यह कैसे काम करता है देखें", bySubscribingText: "सदस्यता लेकर, आप हमारी सहमति देते हैं", trustKicker: "आपका डेटा, आपके निर्णय", trustLine: "अधिक विचारशील रसोई के लिए सरल उपकरण।", termsLink: "नियम व शर्तें", termsKicker: "WASTELYTICS · नियम", close: "बंद करें", footerTagline: "कम बर्बादी। अधिक जानकारी।", restaurantSignup: "रेस्तरां साइन अप", ngoLogin: "NGO लॉगिन", adminSignup: "एडमिन साइन अप", adminLogin: "एडमिन लॉगिन",
    ...
    signUp: "साइन अप", iAmA: "मैं हूँ...", roleRestaurant: "रेस्तरां", roleNgo: "NGO", roleAdmin: "एडमिन", createAccount: "अपना खाता बनाएं", restaurantNamePh: "उदा. ग्रीन टिफिन कंपनी", ngoNamePh: "उदा. रॉबिन हुड आर्मी", adminNamePh: "उदा. प्रिया राव", restaurantNameLabel: "रेस्तरां का नाम", ngoNameLabel: "संगठन का नाम", adminNameLabel: "पूरा नाम", city: "शहर", contactPerson: "संपर्क व्यक्ति", fullName: "पूरा नाम", adminInviteCode: "एडमिन आमंत्रण कोड", locationLink: "स्थान लिंक (Google मानचित्र)", locationHint: "NGO को सबसे छोटा पिकअप मार्ग तय करने में मदद करता है।", ngoNote: "आवेदनों की समीक्षा Wastelytics टीम द्वारा की जाती है। सत्यापित NGO को आमतौर पर 48 घंटों के भीतर जवाब मिल जाता है।", adminNote: "एडमिन एक्सेस के लिए Wastelytics टीम से मान्य आमंत्रण कोड आवश्यक है।", submitting: "सबमिट हो रहा है…", submitVerification: "सत्यापन के लिए सबमिट करें", createAdminAccess: "एडमिन एक्सेस बनाएं", ngoDesc: "सत्यापित NGO आपके शहर के रेस्तरां से अधिशेष भोजन का दावा कर सकते हैं।", adminDesc: "एडमिन एक्सेस केवल मान्य आमंत्रण कोड वाले Wastelytics टीम सदस्यों तक सीमित है।", restaurantDesc: "एक मिनट से भी कम समय में साइन अप करें — मुफ़्त ट्रायल के लिए कार्ड की आवश्यकता नहीं।", ngoHeadline: "अधिशेष बचाएं, अधिक लोगों को खिलाएं।", adminHeadline: "प्लेटफ़ॉर्म को नियंत्रित करें।", restaurantHeadline: "बर्बादी को बचत में बदलें।",
...
function App() {
  ...
  if (role === "landing") return <Landing showLogin={() => setShowLogin(true)} hideLogin={() => setShowLogin(false)} showSignup={(r) => { setShowLogin(false); setSignupRole(r); }} hideSignup={() => setSignupRole(null)} onSignup={handleSignup} onLogin={handleLogin} signup={signupRole} dark={dark} setDark={setDark} lang={lang} setLang={setLang} login={showLogin} showAboutPage={() => setShowAbout(true)} showTermsPage={() => setShowTerms(true)} />;
  return <div className={`shell ${dark ? "theme-dark" : ""}`}><Toaster richColors position="bottom-right" /><aside className={mobileNav ? "sidebar open" : "sidebar"}><div className="brand"><span className="brand-mark"><Leaf size={18} /></span><span>Wastelytics</span></div><div className="workspace"><span className="workspace-dot" />{orgs[role] || "Wastelytics"}<ChevronRight size={14} /></div><nav>{nav.map(({ id, label, icon: Icon }) => <button key={id} data-testid={`nav-${id}`} className={page === id ? "active" : ""} onClick={() => { setPage(id); setMobileNav(false); }}><Icon size={17} />{label}</button>)}</nav><div className="side-foot"><div className="plan-mini"><span>{t.currentPlan}</span><strong>{role === "restaurant" ? t.proPlan : t.verifiedPartnerPlan}</strong><div className="progress"><i /></div></div><button data-testid="logout-button" className="logout" onClick={logout}><LogOut size={16} /> {t.logOut}</button></div></aside><main className="main"><header className="topbar"><button data-testid="mobile-menu-button" className="icon-btn mobile-menu" aria-label="Open navigation" onClick={() => setMobileNav(!mobileNav)}><Menu size={20} /></button><div className="crumb"><span>{role === "restaurant" ? t.restaurantPortal : role === "ngo" ? t.ngoPortal : t.adminPortal}</span><ChevronRight size={14} /><strong>{nav.find((x) => x.id === page)?.label}</strong></div><div className="top-actions">{orgMeta[role]?.location && <a data-testid="topbar-location-link" className="location-pill" href={orgMeta[role].location} target="_blank" rel="noreferrer"><MapPin size={13} /> {orgMeta[role].city || t.viewLocation}</a>}<button data-testid="language-toggle" className="toggle" onClick={() => setLang(lang === "en" ? "hi" : "en")}>{lang === "en" ? "EN" : "हि"}</button><button data-testid="theme-toggle" className="icon-btn" aria-label="Toggle theme" onClick={() => setDark(!dark)}>{dark ? <Sun size={17} /> : <Moon size={17} />}</button><button data-testid="notifications-button" className="icon-btn" aria-label="Notifications" onClick={() => toast.info("You're all caught up — no new alerts")}><Bell size={17} /><i className="notify-dot" /></button><div className="avatar">{initials(orgs[role])}</div></div></header><div className="content">{role === "restaurant" && <RestaurantPage page={page} t={t} logs={logsWithStatus} totalKg={totalKg} addLog={addLog} form={form} setForm={setForm} listings={listings} dark={dark} setPage={setPage} orgMeta={orgMeta.restaurant} orgName={orgs.restaurant} lang={lang} onListSurplus={listSurplus} onShowTerms={() => setShowTerms(true)} />} {role === "ngo" && <NgoPage t={t} page={page} listings={listings} claims={claims} claim={claim} confirmPickup={confirmPickup} verified={session?.profile?.verified} orgName={orgs.ngo} />} {role === "admin" && <AdminPage t={t} page={page} />}</div></main></div>;
}

function Landing({ showLogin, hideLogin, showSignup, hideSignup, onSignup, onLogin, signup, dark, setDark, lang, setLang, login, showAboutPage, showTermsPage }) {
  const t = copy[lang];
  const [tilt, setTilt] = useState({});
  const move = (e) => { const r = e.currentTarget.getBoundingClientRect(); setTilt({ transform: `perspective(800px) rotateX(${(e.clientY - r.top - r.height / 2) / -22}deg) rotateY(${(e.clientX - r.left - r.width / 2) / 22}deg)` }); };
  return <div className={`landing ${dark ? "theme-dark" : ""}`}><Toaster richColors position="bottom-right" /><header className="landing-nav"><div className="brand"><span className="brand-mark"><Leaf size={18} /></span><span>Wastelytics</span></div><div className="landing-links"><a href="#how">{t.navHow}</a><a href="#pricing">{t.navPricing}</a><button data-testid="landing-about-nav" className="text-button nav-link" onClick={showAboutPage}>{t.navAbout}</button><button data-testid="landing-language-toggle" className="toggle" onClick={() => setLang(lang === "en" ? "hi" : "en")}>{lang === "en" ? "EN / हि" : "हि / EN"}</button><button data-testid="landing-theme-toggle" className="icon-btn" onClick={() => setDark(!dark)}>{dark ? <Sun size={17} /> : <Moon size={17} />}</button><button data-testid="landing-ngo-signup" className="text-button" onClick={() => showSignup("ngo")}>{t.ngoSignupBtn}</button><button data-testid="landing-login-button" className="text-button" onClick={showLogin}>{t.login} <ArrowRight size={15} /></button></div></header><section className="hero"><div className="hero-copy"><div className="eyebrow"><span className="pulse" /> {t.heroEyebrow}</div><h1>{t.heroTitle1}<br /><em>{t.heroTitleEm}</em></h1><p>{t.heroSub}</p><div className="hero-actions"><button data-testid="hero-trial-button" className="primary-btn" onClick={() => showSignup("restaurant")}>{t.startTrial} <ArrowRight size={17} /></button><button data-testid="hero-ngo-button" className="secondary-btn" onClick={() => showSignup("ngo")}>{t.joinNgo}</button></div><div className="hero-proof"><div className="proof-avatars"><span>RM</span><span>ST</span><span>AK</span></div><span>{t.heroProof}</span></div></div><div className="hero-ticket-wrap" onMouseMove={move} onMouseLeave={() => setTilt({})}><div className="ticket hero-ticket" style={tilt}><div className="ticket-head"><span>{t.ticketLabel}</span><span className="ticket-status">● {t.ticketLive}</span></div><div className="ticket-main"><small>{t.ticketSavings}</small><strong>₹24,680</strong><div className="ticket-change"><TrendingDown size={14} /> {t.ticketChangeText}</div></div><div className="ticket-line" /><div className="sparkline"><span /><span /><span /><span /><span /><span /><span /><span /><span /><span /></div><div className="ticket-footer"><span>{t.ticketTrendLabel}</span><span>14 SEP 2024</span></div></div></div></section><PartnerMarquee /><section id="how" className="loop-section"><div className="section-kicker">{t.loopKicker}</div><h2>{t.loopTitle1}<br /><span>{t.loopTitleSpan}</span></h2><div className="loop-grid">{t.loopSteps.map(([n, h, p]) => <div className="loop-item" key={n}><span className="loop-num">{n}</span><h3>{h}</h3><p>{p}</p><ArrowRight size={18} /></div>)}</div></section><section id="pricing" className="pricing-section"><div><div className="section-kicker">{t.pricingKicker}</div><h2>{t.pricingTitle1}<br /><span>{t.pricingTitleSpan}</span></h2></div><div className="pricing-grid"><div className="price-ticket"><span className="plan-label">{t.basicLabel}</span><strong>₹100 <small>/ month</small></strong><p>{t.basicDesc}</p><button data-testid="basic-plan-button" onClick={() => showSignup("restaurant")} className="secondary-btn">{t.chooseBasic} <ArrowRight size={15} /></button><ul>{t.basicFeatures.map((f) => <li key={f}><Check size={15} />{f}</li>)}</ul></div><div className="price-ticket featured"><span className="plan-label">{t.proLabel}</span><strong>₹200 <small>/ month</small></strong><p>{t.proDesc}</p><button data-testid="pro-plan-button" onClick={() => showSignup("restaurant")} className="primary-btn">{t.startTrial} <ArrowRight size={15} /></button><ul>{t.proFeatures.map((f) => <li key={f}><Check size={15} />{f}</li>)}</ul></div></div></section><ImpactFacts t={t} /><PartnerNetwork /><AboutSection t={t} showTermsPage={showTermsPage} /><footer className="landing-footer"><div className="brand"><span className="brand-mark"><Leaf size={18} /></span><span>Wastelytics</span></div><span>{t.footerTagline}</span><div className="footer-roles"><button data-testid="footer-restaurant-signup" onClick={() => showSignup("restaurant")}>{t.restaurantSignup}</button><button data-testid="footer-restaurant-login" onClick={showLogin}>{t.login}</button><button data-testid="footer-ngo-login" onClick={showLogin}>{t.ngoLogin}</button><button data-testid="footer-ngo-signup" onClick={() => showSignup("ngo")}>{t.ngoSignupBtn}</button><button data-testid="footer-admin-signup" onClick={() => showSignup("admin")}>{t.adminSignup}</button><button data-testid="footer-admin-login" onClick={showLogin}>{t.adminLogin}</button><button data-testid="footer-about-link" onClick={showAboutPage}>{t.navAbout}</button><button data-testid="footer-terms-link" onClick={showTermsPage}>{t.termsLink}</button></div></footer>{login && <LoginModal t={t} close={hideLogin} onLogin={onLogin} showSignup={showSignup} />}{signup && <SignupModal t={t} role={signup} close={hideSignup} onSignup={onSignup} />}</div>;
}
  ...
  return <div className={`landing about-page ${dark ? "theme-dark" : ""}`}>
    <div className="about-page-inner">
      ...
      <div className="founder-story">
        <p>I'm Yajas Arora, the founder of Wastelytics.</p>
        ...
        <p>That's what Wastelytics is. It's not just a rescue app, and it's not just an analytics tool — it's both, because that's what the problem actually requires.</p>
...
const TERMS_SECTIONS = [
  ...
  { id: "limitation", title: "11. Limitation of Responsibility", paragraphs: ["Wastelytics is provided as a tool to help reduce and redistribute food waste. We do not accept responsibility for illness, loss, or disputes arising from the physical condition of food once it has been listed, claimed, or collected. This platform is a coordination tool — the people actually handling the food are responsible for its safety."] },
  ...
  { id: "contact", title: "14. Contact Us", paragraphs: ["Questions about these terms can be sent to:", "Phone: +91 1800-123-456", "Email: support@wastelytics.app"] },
];
...
function LoginModal({ t, onLogin, close, showSignup }) { const [email, setEmail] = useState(""); const [password, setPassword] = useState(""); const [submitting, setSubmitting] = useState(false); const submit = async () => { if (!email || !password) return toast.error("Enter your email and password"); setSubmitting(true); try { await onLogin(email, password); } catch (err) { toast.error(err.message || "Login failed"); } finally { setSubmitting(false); } }; return <div className="modal-backdrop" onClick={(e) => { if (e.target === e.currentTarget) close(); }}><div className="login-modal"><button data-testid="close-login-button" className="modal-close" aria-label="Close login" onClick={close}><X size={18} /></button><div className="section-kicker">{t.welcomeBack}</div><h2>{t.goodToSeeYou}</h2><p>{t.loginSub}</p><label>{t.emailAddress}<input data-testid="login-email-input" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@restaurant.com" /></label><label>{t.password}<input data-testid="login-password-input" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" /></label><button data-testid="login-submit-button" className="primary-btn full" disabled={submitting} onClick={submit}>{submitting ? t.signingIn : t.enterDashboard} <ArrowRight size={16} /></button><div className="signup-line"><span>{t.newHere}</span><button data-testid="login-signup-link" onClick={() => showSignup && showSignup("restaurant")}>{t.signUp}</button></div></div></div>; }
  ...
  return <div className="modal-backdrop" onClick={(e) => { if (e.target === e.currentTarget) close(); }}>
    <div className="login-modal signup-modal">
      ...
      <form onSubmit={submit}>
        ...
        <label>{t.emailAddress}<input data-testid="signup-email-input" type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder={isNgo ? "hello@ngo.org" : isAdmin ? "you@wastelytics.in" : "you@restaurant.com"} required /></label>
        ...
        {isAdmin && <label>{t.adminInviteCode}<input data-testid="signup-admincode-input" value={form.adminCode} onChange={(e) => setForm({ ...form, adminCode: e.target.value })} placeholder="WL-ADMIN-XXXX" required /></label>}
